...
export default function NutritionSpotlight({ productName, nutriments, processingScore, barcode }: NutritionSpotlightProps) {
  ...
  // Auto-save nutrition spotlight to both products and search history databases when loaded
  useEffect(() => {
    if (insights && barcode) {
      productInsightsManager.saveNutritionSpotlight(barcode, insights);
      searchHistoryInsightsManager.saveNutritionSpotlight(barcode, insights);
    }
  }, [insights, barcode]);
