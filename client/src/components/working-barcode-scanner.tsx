...
export default function WorkingBarcodeScanner({ onScan, isLoading = false }: BarcodeScannerProps) {
  ...
  const detectBarcode = () => {
    ...
    // Get image data from center region where barcode would be
    const centerX = canvas.width / 2;
    const centerY = canvas.height / 2;
    const regionWidth = Math.min(300, canvas.width * 0.8);
    ...
    try {
      ...
      // Simple pattern detection for vertical lines (barcode characteristic)
      const data = imageData.data;
      let verticalLines = 0;
      let transitions = 0;
      
      // Scan horizontally through middle of region
      const midRow = Math.floor(regionHeight / 2);
      let lastPixelDark = false;
      
      for (let x = 0; x < regionWidth; x += 4) {
        const i = (midRow * regionWidth + x) * 4;
        const r = data[i];
        const g = data[i + 1];
        const b = data[i + 2];
      ...
      if (transitions > 20) {
        console.log("Possible barcode detected - transitions:", transitions);
  ...
  return (
    <div className="space-y-10">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">Use your camera to scan or enter barcode manually</p>
      </div>
